...
async function testScenario(scenario) {
  ...
  try {
    // Mock the API call with test data
    const response = await fetch(`${API_BASE}/api/ai-suggestions/test-${scenario.id}`, {
      method: 'POST',
...
function evaluateSuggestions(actual, expected) {
  const evaluation = {
    watering: evaluateCategory(actual.watering, expected.watering),
    fertilizing: evaluateCategory(actual.fertilizing, expected.fertilizing),
    healthScore: evaluateHealthScore(actual.healthScore, expected.healthScore)
  };
  
  // Calculate overall score
  const scores = [evaluation.watering.score, evaluation.fertilizing.score, evaluation.healthScore.score];
...
function evaluateCategory(actual, expected) {
  let score = 0;
  const feedback = [];
...
function evaluateHealthScore(actual, expected) {
